 ...
 * list item
 */
const SortableItem = SortableElement(({ item, song, state, handleItemClick }) => (
  <li className='song-item'>
    <div className='song-item-avatar'>
      <img src={item.get("image")} width='42' />
      <button
        className='song-item-control'
        onClick={() => {
...
const SortableList = SortableContainer(({ list, song, state, handleItemClick }) => {
  return (
    <ul className='list-content'>
      {list.map((item, index) => (
        <SortableItem key={index} state={state} handleItemClick={handleItemClick} item={item} song={song} index={index} />
      ))}
    </ul>
  );
});
...
class Player extends React.Component {
    ...
    return (
      <div className='player'>
        <audio
          ...
          onCanPlay={() => {
            this.props.handleCanPlay(this.props.song);
          }}
        ...
        <Spring
          ...
          onStart={(result) => {
            if (result.value.opacity < 0.5) removeClass(this.$nodeRef, "hide");
          }}
          onRest={(result) => {
            ...
            return (
              ...
              >
                ...
                <div className='song-content'>
                  <div className='cd'>
                    <Spring xy={fullscreen ? [0, 0] : [-X, Y]} scale={fullscreen ? 1 : 0.16} config={config.gentle}>
                      {({ xy, scale }) => {
                        return (
                          <animated.div
                            style={{
                              transform: to([scale.to({ range: [0, 0.65, 0.75, 1], output: [0.16, 0.8, 1.03, 1] }), xy], (scale, xy) => `translate(${xy[0]}px, ${xy[1]}px) scale(${scale})`),
                            }}
                            className='album-cover'
                          >
                            {/* <canvas id="canvas" className="canvas" ref={(canvas) => { this.$canvas = canvas }}></canvas> */}
                            <img
                              src={song.image ? song.image : albumImage}
                              className='album-img'
                              ref={(canvas) => {
                                this.$canvas = canvas;
                              }}
                      ...
                      <div className='lyric-panel'>
                        ...
                        >
                          {lyric.map((item, index) => {
                            return (
                              <p className={classNames("lyric-line", { active: lyricLineNum === index })} key={index}>
                                {item.txt}
                              </p>
                            );
                          })}
        ...
        <div className='controls'>
          ...
          <button
            onClick={() => {
              this.props.handleTogglePlay(this.props.playing);
            }}
          >
            <i
              className={classNames("iconfont toggle", {
                "icon-play": !playing,
                "icon-pause": playing && ready,
              })}
            ></i>
            <i className={classNames("icon-loading hide", { show: playing && !ready })}>
              <Loading />
            </i>
          </button>
        ...
        <div className='progress'>
          ...
          <Slider
            formatter={() => formatTime(currentTime)}
            defaultValue={0}
            value={currentTime * 1000}
            max={duration}
            onAfterChange={this.props.handleAfterTimeChange}
            onChange={(value) => {
              this.props.handleTimeChange(value / 1000);
            }}
          />
        ...
        <div className='controls-right'>
          ...
          <div className='volume-control'>
            <Slider
              ...
              onChange={(value) => {
                this.props.refs.$audio.volume = value;
              }}
            />
          </div>
          <button className='iconfont icon-list' onClick={this.props.handleToggleVisible}></button>
        </div>
        <Spring from={{ translateX: 340 }} to={{ translateX: playlistVisible ? 0 : 340 }} config={{ tension: 500, friction: 25 }}>
          {(styles) => {
            return (
              <animated.div style={styles} className='player-playlist list'>
                <div className='list-header'>
                  <span>当前播放</span> <i className='iconfont icon-close'></i>
                </div>
                <SortableList
                  handleItemClick={this.props.handleItemClick}
                  pressDelay={200}
                  lockAxis='y'
                  helperClass='move-style'
                  state={playing}
                  song={song}
                  list={playlist}
  ...
  componentDidMount() {
    ...
    this.props.initRefs({ $audio: this.$audio, $scroll: this.$scroll, $cover: this.$canvas });
  }
}
